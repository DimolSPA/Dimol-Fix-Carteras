            ...
            success: function (data) {
                if (data != -1) {
                    ...
                    if ($("#Rolid").val() != "0" && $("#Rolid").val() != '') {
                        ...
                        newUrl += "Rolid=" + data;
                        jQuery("#gridDemandados").jqGrid().setGridParam({ url: newUrl }).trigger('reloadGrid', [{ page: 1 }]);
                        fnCargarAsociados();
                        $("#tabRol").show();
                        if (edita > 0) {
        ...
        success: function (chartsdata) {
            ...
            for (var i = 0; i < chartsdata.length; i++) {
                ...
                if (chartsdata[i].Monto == "" && chartsdata[i].RepLegal == "") {
                    contenido = '<div onclick=\"/Cartera/Index/?tipo=J&pag=355\" style=\"cursor:pointer\"><b>' + chartsdata[i].Nombre + '</b><div style="color:red; font-style:italic">' + chartsdata[i].Rut + '</div></div>';
            ...
            '<div style="color:red; font-style:italic">' + chartsdata[i].Tribunal + '</div>' +
...
function fnBotonesGridDescargarDemanda(cellvalue, options, rowobject) {
    return '<div class="tabla">\
                <div class="fila" style="margin-left:36px">\
                    <div class="col">\
                        <button type="button" class="ui-icon ui-icon-arrowthickstop-1-s" style="height:20px;width:19px" onclick="fnDescargarPDFDemandaMasiva(\'' + rowobject + '\')" >Descargar PDF</button>\
                    </div>\
                </div>\
            </div>';
}
...
function fnDescargarPDFDemandaMasiva(id) {
    alert("Aún en desarrollo :p");
    //var datos = id.split(',');
    //$("#TipoDM").val(datos[0]);

    //$('#ppTipoDemandaMasiva').dialog('open');
    //var editor = CKEDITOR.instances['editor1'];
    //if (editor) {
    //    editor.destroy(true);
    //}
...
function fnConfeccionDemandaMasiva() {    
    
    if ($("#TiposDemandaMasiva option:selected").val() != '' && $("#TiposDemandaMasiva option:selected").val() != null) {
        $.ajax({
            type: 'POST',
            url: "/Judicial/ExportToPDFDemandaMasiva", // we are calling json method
            dataType: 'json',
            async: true,
            data: { idPanel: $("#TipoDM").val(), idTemplate: $("#TiposDemandaMasiva").val() },
            success: function (data) {
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                alert('Error al generar la demanda.' + errorThrown);
            }
...
function fnEditarPanelMasivas(id) {
    ...
    if (datos[21] != 0) {
        if (datos[21] != $('#frmAvanceDemanda input[id=GetUsuario]').val()) {
    ...
    $('#frmAvanceDemanda input[id=usridHidden]').val(datos[21]);
    $('#frmAvanceDemanda input[id=rutDeudorHidden]').val(datos[6]);
    $('#frmAvanceDemanda input[id=nombreDeudorHidden]').val(datos[7]);
    $('#frmAvanceDemanda input[id=rutClienteHidden]').val(datos[24]);
    $('#frmAvanceDemanda input[id=nombreClienteHidden]').val(datos[5]);
    $('#frmAvanceDemanda input[id=pclidHidden]').val(datos[22]);
    $('#frmAvanceDemanda input[id=ctcidHidden]').val(datos[23]);
    $('#frmAvanceDemanda input[id=CountFechaEntrega]').val(datos[25]);
    ...
    if (datos[14] == '' || datos[14] == null) {
        ...
        var jsonDate = datos[14].replace(/\D+/g, '');//reemplaza todos los caracteres a excepcion de numeros
        var date = new Date(parseInt(jsonDate)); //jsonDate.substr(6), 10)
    ...
    if (datos[15] == '' || datos[15] == null) {
        ...
        var jsonDate = datos[15].replace(/\D+/g, '');//reemplaza todos los caracteres a excepcion de numeros
        var date = new Date(parseInt(jsonDate)); //jsonDate.substr(6), 10)
...
function fnCursoDemandaMasiva(panelId) {
    //var $optionSelected = $('#gridPanelDemandas tr[id^="' + panelId + '"]').find('input:select:first');
    //console.log($optionSelected.context.activeElement.value);
...
function fnIngresarRol(dateText) {
    if ($('#frmAvanceDemanda input[id=FechaEntrega]').val() == '') {
        ...
        if ($('#frmAvanceDemanda input[id=PanelProcesado]').val() == 'N') {
            ...
            $.ajax({
                type: 'POST',
                url: "/Judicial/TraeDeudorQuiebra/", // we are calling json method
                dataType: 'json',
                async: true,
                data: {
                    rutDeudor: $('#frmAvanceDemanda input[id=rutDeudorHidden]').val()
