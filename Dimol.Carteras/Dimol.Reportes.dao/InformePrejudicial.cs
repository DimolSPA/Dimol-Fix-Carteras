    ...
    public class InformePrejudicial
    {
        ...
        public static void ListarDocumentosDetalle(dto.InformePrejudicial obj)
        {
            try
            {
                ...
                StoredProcedure sp = new StoredProcedure("_Trae_Reporte_Informe_PreJudicial_Judicial"); 
        ...
        public static void ListarDocumentosDetalleOchenta(dto.InformePrejudicial obj)
        {
            try
            {
                decimal monto = 0, saldo = 0, interes = 0, honorarios = 0, gasto = 0;
                DataSet ds = new DataSet();
                StoredProcedure sp = new StoredProcedure("_Trae_Reporte_Informe_PreJudicial_Ochenta");
                sp.AgregarParametro("ccb_codemp", obj.Codemp);
                sp.AgregarParametro("ccb_pclid", obj.Pclid);
                sp.AgregarParametro("ccb_tipcart", obj.TipoCartera);
                sp.AgregarParametro("idi_idid", obj.Idioma);
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        .Select(row => new
                        {
                            ...
                            Pclid = row.Field<decimal>("ccb_pclid"),
                            Negocio = row.Field<string>("ccb_numesp"),
                            Sii = row.Field<string>("sii"),
                            PoderJud = TraeNumeroCausasRut(row.Field<decimal>("ccb_ctcid")),
                            Terreno = row.Field<int>("terreno")
                        ...
                        foreach (var asegurado in lstAsegurados)
                        {
                            ...
                            if (deudor == null)
                            {
                                deudor = new dto.InformePrejudicialDeudor
                                {
                                    ...
                                    Pclid = asegurado.Pclid,
                                    Sii = asegurado.Sii,
                                    PoderJud = asegurado.PoderJud,
                                    Terreno = asegurado.Terreno
                                };
                                insertaDeudor = true;
                            }
                        ...
                        saldo = 0;
                        List<dto.InformePrejudicialDeudor> lstDocs = new List<dto.InformePrejudicialDeudor>();

                        foreach (dto.InformePrejudicialDeudor d in obj.lstDocumentos.OrderByDescending(o=>o.Totales.Saldo))
                        {
                            saldo += d.Totales.Saldo;

                            if(saldo <= (obj.Totales.Saldo*((decimal)0.8)))
                            {
                                lstDocs.Add(d);
                            }
                        ...
                        obj.lstDocumentos = lstDocs.OrderByDescending(o => o.Totales.Saldo).ToList();
        ...
        public static int TraeNumeroCausasRut(decimal rut)
        {
            try
            {
                DataSet ds = new DataSet();
                StoredProcedure sp = new StoredProcedure("_Trae_Numero_Causas_Rut");
                sp.AgregarParametro("rut", Int32.Parse(rut.ToString()));
                ds = sp.EjecutarProcedimiento();
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        return Int32.Parse(ds.Tables[0].Rows[0]["causas"].ToString());
                    }
                    else
                    {
                        return 0;
