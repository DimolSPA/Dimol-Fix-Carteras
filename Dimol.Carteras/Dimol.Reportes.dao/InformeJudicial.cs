...
namespace Dimol.Reportes.dao
{
    public class InformeJudicial
    {
                        ...
                        foreach (DataRow detalle in ds.Tables[0].Rows)
                        {
                            ...
                            if (causa == null)
                            {
                                causa = new dto.InformeJudicialCausa()
                                {
                                    ...
                                    Estado = detalle["ect_nombre"].ToString(), //detalle["eci_nombre"].ToString(),
        ...
        public static void ListarDocumentosInformeJudicialOchenta(dto.InformeJudicial obj)
        {
            try
            {
                decimal capital = 0, gasto = 0, total = 0;
                int cantidad = 0;
                DataSet ds = new DataSet();
                StoredProcedure sp = new StoredProcedure("_Trae_Reporte_Informe_Judicial_Ochenta");
                sp.AgregarParametro("rol_codemp", obj.Codemp);
                sp.AgregarParametro("rol_pclid", obj.Pclid);
                sp.AgregarParametro("eci_idid", obj.Idioma);
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow detalle in ds.Tables[0].Rows)
                        {
                            ...
                            if (causa == null)
                            {
                                causa = new dto.InformeJudicialCausa()
                                {
                                    ...
                                    Resumen = detalle["rle_comentario"].ToString(),
                                    UltimaGestion = DateTime.Parse(detalle["rle_fecha"].ToString()),
                                    Sii = detalle["sii"].ToString(),
                                    PoderJud = dao.InformePrejudicial.TraeNumeroCausasRut(decimal.Parse(detalle["rol_ctcid"].ToString())),
                                    Terreno = Int32.Parse(detalle["terreno"].ToString())
                                };
                                insertaCausa = true;
                            }
                        ...
                        List<dto.InformeJudicialCodigoCarga> lstDocs = new List<dto.InformeJudicialCodigoCarga>();
                        

                        foreach (dto.InformeJudicialCodigoCarga d in obj.lstCodigoCarga.OrderByDescending(o => o.Totales.Total))
                        {
                            List<dto.InformeJudicialAsegurado> lstAseg = new List<dto.InformeJudicialAsegurado>();

                            foreach (dto.InformeJudicialAsegurado a in d.lstAsegurados.OrderByDescending(p => p.Totales.Total)) {

                                List<dto.InformeJudicialDeudor> lstDeu = new List<dto.InformeJudicialDeudor>();

                                foreach(dto.InformeJudicialDeudor deu in a.lstDeudores.OrderByDescending(p => p.Totales.Total)) { 

                                    total += deu.Totales.Total;

                                    if (total <= (obj.Totales.Total * ((decimal)0.8)))
                                    {
                                        lstDeu.Add(deu);
                                    }
                                }

                                lstAseg.Add(new dto.InformeJudicialAsegurado {
                                    SubCartera = a.SubCartera,
                                    RutSubCartera = a.RutSubCartera,
                                    lstDeudores = lstDeu,
                                    Totales = a.Totales,
                                    lstCausas = a.lstCausas
                                });

                            }
                            lstAseg = lstAseg.OrderByDescending(p => p.Totales.Total).ToList();

                            lstDocs.Add(new dto.InformeJudicialCodigoCarga {
                                Codid = d.Codid,
                                CodigoCarga = d.CodigoCarga,
                                lstAsegurados = lstAseg,
                                Totales = d.Totales,
                                lstDeudores = d.lstDeudores
                            });
                        }

                        obj.lstCodigoCarga = lstDocs.OrderByDescending(o => o.Totales.Total).ToList();
