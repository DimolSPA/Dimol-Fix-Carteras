

Create Procedure Update_ProvCli_Sucursal(@pcs_codemp integer, @pcs_pclid numeric (15), @pcs_pcsid integer, @pcs_nombre varchar (150),
                                                                       @pcs_comid integer, @pcs_direccion varchar (500), @pcs_telefono varchar (100), @pcs_fax varchar (100),
                                                                       @pcs_mail varchar (100), @pcs_casamatriz char (1), @pcs_bcoid integer, @pcs_tipcta char (1), 
                                                                       @pcs_numcta varchar (20), @pcs_codigo varchar(10)) as
  UPDATE provcli_sucursal  
     SET pcs_nombre = @pcs_nombre,   
         pcs_comid = @pcs_comid,   
         pcs_direccion = @pcs_direccion,   
         pcs_telefono = @pcs_telefono,   
         pcs_fax = @pcs_fax,   
         pcs_mail = @pcs_mail,   
         pcs_casamatriz = @pcs_casamatriz,   
         pcs_bcoid = @pcs_bcoid,   
         pcs_tipcta = @pcs_tipcta,   
         pcs_numcta = @pcs_numcta,
         pcs_codigo = @pcs_codigo 
   WHERE ( provcli_sucursal.pcs_codemp = @pcs_codemp ) AND  
         ( provcli_sucursal.pcs_pclid = @pcs_pclid ) AND  
         ( provcli_sucursal.pcs_pcsid = @pcs_pcsid )
