...
Create Procedure Update_Stock_Productos_Stock(@bsp_codemp integer, @bsp_bodid integer, @bsp_bdsid integer, @bsp_bscid varchar(10),
  ...
  UPDATE   productos_stock
   ...
   WHERE ( productos_stock.pst_codemp =  @bsp_codemp ) AND  
         ( productos_stock.pst_prodid = @bsp_prodid )   
  ...
  UPDATE bodegas_sector_productos  
   ...
   WHERE ( bodegas_sector_productos.bsp_codemp = @bsp_codemp ) AND  
         ...
         ( bodegas_sector_productos.bsp_bscid = bsp_bscid ) AND  
         ( bodegas_sector_productos.bsp_posicion = @bsp_posicion ) AND  
         ( bodegas_sector_productos.bsp_prodid = @bsp_prodid )
