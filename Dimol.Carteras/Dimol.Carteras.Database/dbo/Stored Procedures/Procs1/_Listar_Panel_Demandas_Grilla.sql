CREATE PROCEDURE [dbo].[_Listar_Panel_Demandas_Grilla]
(
@codemp int,
@where varchar(1000),
@sidx varchar(255),
...
set @query = @query + 'SELECT DISTINCT PD.PANEL_ID PanelId, PD.PROCESADA,
	...
	(select TDOC.TPC_NOMBRE from TIPOS_CPBTDOC TDOC with (nolock) where TDOC.TPC_CODEMP = PD.CODEMP AND TDOC.TPC_TPCID = PD.TPCID) TipoDocumento,
	 ...
	 (select (count(PANEL_ID)) from PANEL_DEMANDA_CORRECCION_HISTORIAL where PANEL_ID = PD.PANEL_ID) countFechaEntrega,
	 CASE 
	...
	PD.CURSODEMANDA, 
	(select count(PANEL_ID) from PANEL_DEMANDA_CURSODEMANDA_HISTORIAL where PANEL_ID = PD.PANEL_ID and CURSODEMANDA = ''NO'') CountCursoDemanda,
...
LEFT JOIN PANEL_DEMANDA_DETALLE PDDE with (nolock) ON PD.PANEL_ID = PDDE.PANEL_ID
