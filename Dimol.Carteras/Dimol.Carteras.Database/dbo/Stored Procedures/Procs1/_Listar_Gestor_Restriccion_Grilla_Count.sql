...
-- =============================================
CREATE PROCEDURE [dbo].[_Listar_Gestor_Restriccion_Grilla_Count]
(
@codemp int,
@codsuc int,
@where varchar(1000),
...
set @query = '  select count(Usrid) count from
  (select *,ROW_NUMBER() OVER (ORDER BY Usrid asc) as row from    
  ('
