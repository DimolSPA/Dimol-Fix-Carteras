CREATE PROCEDURE [dbo].[_Listar_PanelAlerta_Encargados_Grilla]
(
@codemp int,
@where varchar(1000),
@sidx varchar(255),
...
set @query = 'WITH DemandaEncargados AS (
		SELECT PD.PANEL_ID,
			...
			CASE WHEN 
				...
				< 0 THEN 0 ELSE dbo._Trae_DiasSemana(PDDE.FEC_ENVIO, isnull(PDDE.FEC_ENTREGA, GETDATE())) END As DiasTranscurso
...
select * from
  (select *,ROW_NUMBER() OVER (ORDER BY USR ' + @sord+ ') as row from    
  (	' 
  
set @query = @query + '
SELECT
	CASE WHEN USR IS NULL THEN 0 ELSE USR END as USR,
	CASE WHEN USR IS NULL THEN ''NO ASIGNADO'' ELSE Encargado END as Encargado, 
