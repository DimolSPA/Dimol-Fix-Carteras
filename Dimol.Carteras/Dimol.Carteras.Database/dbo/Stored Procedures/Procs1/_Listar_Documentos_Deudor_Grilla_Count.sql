...
-- =============================================
CREATE PROCEDURE [dbo].[_Listar_Documentos_Deudor_Grilla_Count]
(
@codemp int ,
@idioma int ,
@ctcid int ,
...
set @query = '  select count(Dcdid) count from
  (select *,ROW_NUMBER() OVER (ORDER BY Pclid,Ctcid) as row from    
  (	' 
