CREATE Procedure [dbo].[_Listar_Anula_Pagos_Grilla_Count]
(@codemp integer, 
@sucid integer, 
@pclid integer, 
@ctcid numeric(15), 
...
set @query = '  select count(NumeroAplicacion) count from
  (select *,ROW_NUMBER() OVER (ORDER BY Anio asc) as row from    
  (	' 
...
begin
  set @query = @query + 'SELECT apl_anio Anio, 
								...
								case api_vdeid when null then 0 else api_vdeid end as vdeid, 
								numcta NumeroCuenta2
 from #Apl
