...
-- =============================================
CREATE PROCEDURE [dbo].[_Listar_Comprobante_Compra_Grilla]
(
@codemp int,
@idioma int,
@codsuc int,
...
CASE tci_nombre
WHEN ''BOLETA HONORARIOS MANUAL'' THEN ''BHM''
WHEN ''BOLETA HONORARIOS ELECTRONICA'' THEN ''BHE''
ELSE tci_nombre END AS TipoDocumento,   
...
and detalle_comprobantes_rol.dcr_numero = vcc.cbc_numero) RutDeudor, 
(select top 1 CTC_NOMFANT 
from DEUDORES left join rol 
on rol_ctcid = ctc_ctcid 
and rol_codemp = ctc_codemp 
left join detalle_comprobantes_rol 
...
and detalle_comprobantes_rol.dcr_sucid = vcc.cbc_sucid 
and detalle_comprobantes_rol.dcr_tpcid = vcc.cbc_tpcid 
and detalle_comprobantes_rol.dcr_numero = vcc.cbc_numero) NombreDeudor, 
(select top 1 rol_numero from rol left join detalle_comprobantes_rol 
on rol.ROL_CODEMP = detalle_comprobantes_rol.DCR_CODEMP 
and rol.ROL_ROLID = detalle_comprobantes_rol.DCR_ROLID  
where detalle_comprobantes_rol.dcr_codemp = vcc.cbc_codemp 
and detalle_comprobantes_rol.dcr_sucid = vcc.cbc_sucid 
and detalle_comprobantes_rol.dcr_tpcid = vcc.cbc_tpcid 
and detalle_comprobantes_rol.dcr_numero = vcc.cbc_numero) Rol, 
(select top 1 trb_nombre from tribunales left join rol 
...
and detalle_comprobantes_rol.dcr_sucid = vcc.cbc_sucid 
and detalle_comprobantes_rol.dcr_tpcid = vcc.cbc_tpcid 
and detalle_comprobantes_rol.dcr_numero = vcc.cbc_numero) Tribunal 
...
--If @numero_interno is not null begin
--    set @query =@query + ' and cbc_numero= '+@numero_interno
--End
