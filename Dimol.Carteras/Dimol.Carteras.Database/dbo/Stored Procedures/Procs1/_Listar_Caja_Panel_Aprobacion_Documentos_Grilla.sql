CREATE PROCEDURE [dbo].[_Listar_Caja_Panel_Aprobacion_Documentos_Grilla]
(
@codemp int,
@where varchar(1000),
@sidx varchar(255),
...
set @query = @query + 'select distinct
	...
	CASE WHEN crd.CRITERIO_ID = 5 then 
                                     CASE when crd.MONTO_FACTURAR > 0 then NULL  
                                      else crd.OBSERVACIONES END 
    ELSE crd.OBSERVACIONES END AS ''OBSERVACIONES'',
	--crd.OBSERVACIONES,
