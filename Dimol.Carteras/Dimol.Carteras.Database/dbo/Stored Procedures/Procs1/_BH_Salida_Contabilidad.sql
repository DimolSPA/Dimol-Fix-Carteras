...
-- =============================================
CREATE PROCEDURE [dbo].[_BH_Salida_Contabilidad] (@codemp int , @desde datetime , @hasta datetime)
AS
BEGIN
	SET NOCOUNT ON;
...
, dbo._Formato_Rut_zero(p.PCL_RUT) + ', ' + p.PCL_NOMFANT + ', ' +
case cc.CBC_TPCID when 35 then 'BHM' when 59 then 'BHE' end  + ', ' + cc.CBC_NUMPROVCLI
 [Comentario]
,'HP' [TipoDeHonorario]
