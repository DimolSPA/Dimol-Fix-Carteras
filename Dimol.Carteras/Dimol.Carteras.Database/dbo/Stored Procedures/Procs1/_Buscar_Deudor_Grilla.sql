...
-- =============================================
CREATE PROCEDURE [dbo].[_Buscar_Deudor_Grilla]
(
@codemp int ,
@sucid int ,
@usrid int ,
...
set @query = @query + 'SELECT provcli.pcl_pclid pclid,
	   ...
	   '''' as Rol,
	   provcli.PCL_TIPCLI TipoCliente
...
set @query = ' select * from
  (select *,ROW_NUMBER() OVER (ORDER BY ' + @sidx + ' ' + @sord+ ') as row from    
  (SELECT view_rol.rol_pclid as pclid,  
                 view_rol.pcl_nomfant NombreCliente,  
                  view_rol.rol_ctcid as ctcid, ctc_rut as Rut, 
                  view_rol.ctc_nomfant NombreFantasia,  
                    '''' as Gestor,'''' as gesid, rol_numero Rol
                   FROM view_rol
