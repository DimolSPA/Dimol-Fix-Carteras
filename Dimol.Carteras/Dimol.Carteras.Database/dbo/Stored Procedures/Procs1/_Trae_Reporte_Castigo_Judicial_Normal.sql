CREATE procedure [dbo].[_Trae_Reporte_Castigo_Judicial_Normal](@cbc_codemp integer, @cbc_sucid integer, @cbc_tpcid integer,  @cbc_desde numeric(15), @cbc_hasta numeric(15), @ctcid numeric(15), @idi_idid integer) as
 SELECT DISTINCT view_cabecera_comprobantes.cbc_pclid,   
   ...
   WHERE ( provcli_sucursal.pcs_comid = view_datos_geograficos_a.com_comid ) and  
         ...
         ( isnull(view_cabecera_comprobantes.cbc_pcsid, 1) = provcli_sucursal.pcs_pcsid ) and  
		 ...
		 ( cartera_clientes_documentos_cpbt_doc.ccb_ctcid = @ctcid ) AND 
