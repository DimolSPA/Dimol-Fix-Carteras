...
CREATE Procedure [dbo].[_Trae_Reporte_Liquidacion_Parcial](@ccb_codemp integer, @ccb_pclid integer, @ccb_ctcid integer, @ccb_tipcart integer, @ccb_estcpbt char(1), @idioma integer, @gsc_sucid integer, @ccb_ccbid varchar(8000)) as
  
declare @query varchar(8000);

set @query = 'SELECT cartera_clientes_documentos_cpbt_doc.ccb_pclid,   
   ...
   WHERE ( ciudad.ciu_ciuid = comuna.com_ciuid ) and  
         ...
         ( ( cartera_clientes_documentos_cpbt_doc.ccb_codemp = ' + CONVERT(VARCHAR,@ccb_codemp) + ' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.ccb_pclid = ' + CONVERT(VARCHAR,@ccb_pclid) + ' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.ccb_ctcid = ' + CONVERT(VARCHAR,@ccb_ctcid) + ' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.ccb_tipcart = ' + CONVERT(VARCHAR,@ccb_tipcart) + ' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.ccb_estcpbt = ''' + CONVERT(VARCHAR,@ccb_estcpbt) + ''' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.tci_idid = ' + CONVERT(VARCHAR,@idioma) + ' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.eci_idid = ' + CONVERT(VARCHAR,@idioma) + ' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.mci_idid = ' + CONVERT(VARCHAR,@idioma) + ' ) AND  
         ( cartera_clientes_documentos_cpbt_doc.ccb_ccbid in (' + CONVERT(VARCHAR(8000),@ccb_ccbid) + ') ) AND 
