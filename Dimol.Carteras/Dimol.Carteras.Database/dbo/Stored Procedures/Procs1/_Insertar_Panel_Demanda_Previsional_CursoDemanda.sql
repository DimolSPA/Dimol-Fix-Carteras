CREATE PROCEDURE [dbo].[_Insertar_Panel_Demanda_Previsional_CursoDemanda]
(
	@codemp int,
	@panelId int,
...
declare
	...
	SET @cursoDemandaId = (SELECT IsNull(Max(ID)+1, 1)
									FROM PANEL_DEMANDA_PREVISIONAL_CURSODEMANDA_HISTORIAL)
	INSERT INTO PANEL_DEMANDA_PREVISIONAL_CURSODEMANDA_HISTORIAL(PANEL_ID, ID, CURSODEMANDA,MOTIVO, USRID_REGISTRO)
	...
	begin
		...
		set @rolId =  (select rolid from PANEL_DEMANDA_PREVISIONAL_DETALLE where PANEL_ID = @panelId)
