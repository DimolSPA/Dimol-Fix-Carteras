...
Create Procedure Trae_Cartera_Clientes_CpbtDoc_Ingresos(@ccb_codemp integer, @ccb_pclid integer, @idi_idid integer, @desde datetime, @hasta datetime, @ccb_tipcart smallint) as
  SELECT cartera_clientes_documentos_cpbt_doc.pcl_rut,   
   ...
   WHERE ( cartera_clientes_documentos_cpbt_doc.tci_idid = idiomas.idi_idid ) and  
         ...
         ( cartera_clientes_documentos_cpbt_doc.ccb_estcpbt <> 'X' and ccb_fecing >= @desde and ccb_fecing <= @hasta and ccb_tipcart = @ccb_tipcart  )   
         ) 
