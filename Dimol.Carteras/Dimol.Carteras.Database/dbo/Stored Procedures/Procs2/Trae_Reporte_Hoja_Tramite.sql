...
Create Procedure Trae_Reporte_Hoja_Tramite(@rol_codemp integer, @rol_pclid integer, @idi_idid integer) as
  SELECT DISTINCT view_rol_datos.rol_numero,   
         ...
         substring(rle_comentario, 1, 2000) as Comentario,
         esj_orden,
         pcl_nomfant,
         reg_nombre,
         rol_fecultgest,
   ...
   WHERE ( view_rol_datos.rol_codemp = rol_estados.rle_codemp ) and  
         ...
         ( idiomas.idi_idid = @idi_idid )   and esj_esjid > 0
         )
