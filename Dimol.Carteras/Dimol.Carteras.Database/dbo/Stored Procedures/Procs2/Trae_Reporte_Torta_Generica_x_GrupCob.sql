...
Create Procedure Trae_Reporte_Torta_Generica_x_GrupCob(@ccb_codemp integer, @ccb_pclid integer, @ccb_tipcart integer, @ect_prejud char(1), @idi_idid integer, @grc_sucid integer, @grc_grcid integer) as
  SELECT cartera_clientes_documentos_cpbt_doc.ccb_pclid,   
   ...
   WHERE ( cartera_clientes_documentos_cpbt_doc.ccb_codemp = @ccb_codemp ) AND  
         ...
         ( cartera_clientes_documentos_cpbt_doc.ccb_ctcid = gsc_ctcid ) AND  
         ( gcg_codemp = gsc_codemp ) AND  
         ( gcg_sucid = gsc_sucid ) AND  
         ( gcg_gesid = gsc_gesid  ) AND  
         ( gcg_codemp = grc_codemp ) AND  
         ( gcg_sucid = grc_sucid ) AND  
         ( gcg_grcid = grc_grcid ) AND  
         ( ccb_estcpbt in('F', 'V','J') and tci_idid =  @idi_idid and eci_idid  = @idi_idid and mci_idid =  @idi_idid and ccb_tipcart = @ccb_tipcart and ect_prejud in ( @ect_prejud, 'A') and grc_sucid = @grc_sucid and grc_grcid = @grc_grcid  )
