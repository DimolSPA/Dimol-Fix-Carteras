...
Create Procedure Trae_Reporte_Torta_Generica_CodCarga(@ccb_codemp integer, @ccb_pclid integer, @ccb_tipcart integer, @ect_prejud char(1), @idi_idid integer, @pcc_codid integer) as
  SELECT cartera_clientes_documentos_cpbt_doc.ccb_pclid,   
   ...
   WHERE ( cartera_clientes_documentos_cpbt_doc.ccb_codemp = @ccb_codemp ) AND  
         ...
         ( ccb_estcpbt in('F', 'V','J') and tci_idid =  @idi_idid and eci_idid  = @idi_idid and mci_idid =  @idi_idid and ccb_tipcart = @ccb_tipcart and ect_prejud in ( @ect_prejud, 'A') and pcc_codid = @pcc_codid )
