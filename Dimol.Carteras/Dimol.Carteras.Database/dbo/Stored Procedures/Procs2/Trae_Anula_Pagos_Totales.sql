...
Create Procedure Trae_Anula_Pagos_Totales(@apl_codemp integer, @apl_sucid integer, @ddi_pclid integer, @ddi_ctcid numeric(15), @idi_idid integer) as
  SELECT aplicaciones.apl_codemp,   
   ...
   WHERE ( aplicaciones_items.api_codemp = aplicaciones.apl_codemp ) and  
         ...
         and apl_sucid = @apl_sucid and apl_accion =-1 and ddi_pclid = @ddi_pclid and ddi_ctcid = @ddi_ctcid and
        idi_idid = @idi_idid
...
insert into #Apl 
 SELECT aplicaciones.apl_codemp,   
   ...
   WHERE ( aplicaciones_items.api_codemp = aplicaciones.apl_codemp ) and  
         ...
         and apl_sucid = @apl_sucid and apl_accion =-1 and ccb_pclid = @ddi_pclid and ccb_ctcid = @ddi_ctcid and
          idi_idid = @idi_idid
...
insert into #Apl  
 ...
 and apl_codemp = @apl_codemp
         and apl_sucid = @apl_sucid and apl_accion =-1 and cbc_pclid = @ddi_pclid and api_ctcid = @ddi_ctcid and
          idi_idid = @idi_idid



   SELECT sum(api_capital), sum(api_interes), sum(api_honorario), sum(api_gastpre), sum(api_gastjud), sum(api_capital + api_interes + api_honorario + api_gastpre + api_gastjud)
 from #Apl
where tci_nombre + '_' + ddi_numcta  not in (select tci_nombre + '_' + ddi_numcta FROM view_aplicaciones where apl_codemp = @apl_codemp and ddi_pclid = @ddi_pclid and ddi_ctcid = @ddi_ctcid 
