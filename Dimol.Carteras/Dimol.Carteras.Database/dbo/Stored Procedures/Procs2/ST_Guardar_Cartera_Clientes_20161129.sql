create  Procedure [dbo].[ST_Guardar_Cartera_Clientes_20161129](@codemp integer, @pclid numeric (15), @ID_CARGA numeric (15), @CODIGO_CARGA integer,
...
begin
	...
	set @ctcid = (  SELECT IsNull(Max(ctc_ctcid)+1, 1)     FROM deudores  with (nolock)   WHERE ( deudores.ctc_codemp = @codemp ))
	
	
	select top 1 @direccion = direccion, @comid = (SELECT TOP 1 C.COM_COMID comid FROM SITREL_COMUNA ST  with (nolock) , COMUNA C   with (nolock) 
...
SELECT  s.rut,
	...
	s.anexo,
	s.codigo_area
	FROM sitrel_telefono s  with (nolock) --left join [DEUDORES_TELEFONOS_SITREL] d on d.ctcid = @ctcid and CONVERT(INT,d.numero) = CONVERT(INT,s.numero)
	where s.id_carga = @id_carga 
	and s.rut = @rut
	and s.PCLID = @pclid
open cur_telefonos
...
begin
	...
	if not exists (select 1 from [DEUDORES_TELEFONOS_SITREL]  with (nolock)  where CTCID = @ctcid and LTRIM(RTRIM([NUMERO])) = LTRIM(RTRIM(@NUMERO)))
	begin
	begin try
...
select s.rut,s.email
	from sitrel_email s  with (nolock) --left join deudores_mail_sitrel d on d.ctcid = @ctcid and LTRIM(RTRIM(d.mail)) = LTRIM(RTRIM(s.email))
	 where s.id_carga = @id_carga 
	and s.rut = @rut
	and s.PCLID = @pclid
open cur_email
...
begin
	...
	set @ctcid = (  SELECT ctc_ctcid  FROM deudores   WHERE ctc_codemp = @codemp and ctc_numero = @rut )
	  ...
	  UPDATE cartera_clientes_cpbt_doc SET
	   ...
	   --ccb_estcpbt = @estcpbt,
	   --CCB_ESTID = 1, -- sin gestionar @estid,
